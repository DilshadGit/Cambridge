...
def contact(request):
    ...
    if form.is_valid():
        ...
        if send_mail(subject, message, form_email, to_email, fail_silently=True):
            # Just take the fist name of the person to pass to the redirect
            # page
            first_name = full_name.split()[0]
            return HttpResponseRedirect((reverse('contact:contact_thanks_email', kwargs={'name': first_name})))
        else:
            logger.error("We were unable to send an email to contact: {} at email address: {}".format(
                full_name, form_email))
            pass
